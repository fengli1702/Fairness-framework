...
def centerize_ranks(df):
    # 对 'fairness_id' 进行排名并生成 'centerized_fairness_id'
    df['centerized_fairness_id'] = df.groupby('group_id')['fairness_id'].rank(method='min').astype(int)

    # 对 'theta' 进行排名并生成 'centerized_theta'
    # 先进行排名，处理 NA 或 Inf 的值
    df['centerized_theta'] = df.groupby('group_id')['theta'].rank(method='max', ascending=False)
    
    # 处理可能的 NaN 或 Inf 值，替换为-1或其他有效值
    df['centerized_theta'] = df['centerized_theta'].fillna(-1)
    df['centerized_theta'] = df['centerized_theta'].replace([np.inf, -np.inf], -1)

    # 转换为整数类型
    df['centerized_theta'] = df['centerized_theta'].astype(int)
