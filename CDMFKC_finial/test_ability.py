...
path = "../data/test/trainfinial.csv"
...
valid_data = pd.read_csv("../data/test/test_finial_filtered.csv")
valid_data['common_knowledge'] = [[] for _ in range(len(valid_data))]

test_data = pd.read_csv("../data/test/test_finial_filtered.csv")
...
user_n = np.max(train_data['fairness_id'])
...
cdm = CDMFKC(user_n+1,item_n+1,knowledge_n,123)
cdm.train(train_set, valid_set, epoch=10, device="cuda")
cdm.save("cdmfkc.snapshot")
...
all_virtual_user_data = pd.read_csv('../data/test/trainfinial.csv')
...
test_fairness = transform2(
    all_virtual_user_data["user_id"] -1,
