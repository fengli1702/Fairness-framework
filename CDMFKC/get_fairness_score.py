...
def centerize_ranks(df, theta_col):
    df['centerized_fairness_id'] = df.groupby('group_id')['fairness_id'].rank(method='min').astype(int)
    df['centerized_theta'] = df.groupby('group_id')[theta_col].rank(method='max', ascending=False).astype(int)
    return df
...
# 获取所有 theta 列
theta_columns = [col for col in df.columns if col.startswith('theta_')]

# 计算每个 theta 列的公平性指标
for theta_col in theta_columns:
    df = centerize_ranks(df, theta_col)
