...
# 初始化模型
cdm =  KSCD_IF(dropout=0.5, knowledge_num=knowledge_n, latent_dim=64,stu_num=user_n,exer_num=item_n, device="cuda", dtype=torch.float32)
cdm.train(train_data=train_set, test_data=valid_set, epoch=10, device="cuda")

print("Training finished.")

# 保存模型
cdm_path = "kscd.snapshot"
torch.save(cdm.state_dict(), cdm_path)
print(f"Model saved to {cdm_path}.")
...
with open("test_acc.txt", "a") as f:
    f.write(f"\n{path}\n")
    f.write(f"KSCD: Test AUC: {auc:.6f}, Accuracy: {accuracy:.6f}\n")
